// This file has been modified from its original Common.Logging source to make it compatible with .NET standard
...
namespace Common.Logging.NLog
{
    ...
    {
        // AnimeRecs modification: System.Collections.Specialized.NameValueCollection does not exist in .NET standard.
        ...
        //    : this(ToCommonLoggingCollection(properties))
        //{ }
            ...
            : base(true)
        {
            ...
            if (properties != null)
            {
                ...
                if (properties["configFile"] != null)
                {
                    ...
                    {
                        // AppDomain class does not exist in .NET standard
                        //configFile = Path.Combine(AppDomain.CurrentDomain.BaseDirectory.TrimEnd('/', '\\') + "/", configFile.Substring(2));
                        configFile = Path.Combine(AppContext.BaseDirectory.TrimEnd('/', '\\') + "/", configFile.Substring(2));
                    }
                }
